...
export default function Timeline() {
  ...
  return (
    <section id="timeline" className="py-20 border-t border-white/10">
      <motion.div
        ...
        <div className="relative border-l border-white/10 ml-4 md:ml-6 space-y-12">
          {timelineEvents.map((event, index) => (
            ...
            >
              {/* Timeline dot */}
              <div 
                className={`absolute w-4 h-4 rounded-full ${event.color} ${event.glow} -left-[8.5px] top-6 border-4 border-[#0a0a0a] group-hover:scale-125 transition-transform`} 
              />
              
              <div className="bg-white/5 border border-white/10 p-6 rounded-2xl hover:bg-white/10 transition-colors">
                <div className="flex items-center gap-3 mb-2">
                  <event.icon className="text-zinc-400" size={20} />
